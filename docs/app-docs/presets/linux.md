...
**Note**: After uploading, you cannot edit the preset's tracking settings. Metadata (name, description, tags, games) can be edited if you're the author. Preset files are stored locally in `~/.local/share/lookpilot/presets/` and can be manually backed up or shared as JSON files.
...
- Preset files are stored in `~/.local/share/lookpilot/presets/` and can be manually copied for backup
...
- Keybinds may not work when using Wayland
- Check if another application is capturing the key combination
