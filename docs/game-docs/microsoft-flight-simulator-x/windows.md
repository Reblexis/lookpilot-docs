...
Microsoft Flight Simulator X is **more involved** than games that only need a single LookPilot protocol. External head pose is delivered through **SimConnect**, not LookPilot’s FreeTrack-style protocol. The supported approach is the same as other SimConnect-based setups (for example [Beam Eye Tracker’s FSX: Steam Edition guide](https://beam.eyeware.tech/games/microsoft-flight-simulator-x-steam-edition/)): **LookPilot → opentrack → Microsoft FSX SimConnect**.
...
5. Launch FSX. After the first-time setup of this chain, **fully quit and restart** the sim if it does not react—FSX often only notices SimConnect clients after a clean launch.
